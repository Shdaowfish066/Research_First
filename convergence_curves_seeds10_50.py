"""
=============================================================================
PER-EPOCH CONVERGENCE CURVES, INDEPENDENT SEED SET  (Fig. 2 replication)
=============================================================================
convergence_curves_5seed.py produced the published Fig. 2 / Table IX using
seeds 42, 0, 1, 2, 3 -- a set that includes seed 42, the seed the single-seed
table was already built on, so those means are not an independent check of the
epoch-1 ranking. This reruns the identical experiment on a disjoint seed set
(10, 20, 30, 40, 50), the same seeds used for the 100-cell variance study, so
the epoch-1 smooth-vs-ReLU gap and the ranking can be confirmed on data that
shares no seed with the original.

PIPELINE FIDELITY: byte-for-byte identical to convergence_curves_5seed.py
apart from SEEDS and OUT -- same PaperGRU (dropout= argument present,
o[:,-1,:] readout), same ONLY torch.manual_seed and np.random.seed with no
cudnn flags. Setting cudnn.deterministic perturbs the numerics enough to flip
epoch-1 outcomes, because epoch 1 is a knife-edge: the model either escapes
the ~50% plateau in one pass or it does not.

Output: results/convergence_curves_seeds10_50.csv
...
AFS = ["ReLU", "ELU", "LeakyReLU", "Tanh", "GELU", "SiLU", "Mish", "SELU",
       "PReLU", "Hardswish"]
OUT = "results/convergence_curves_seeds10_50.csv"
