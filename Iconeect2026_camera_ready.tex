...
The activation function literature evolved separately. Nair and
Hinton~\cite{nair2010} introduced ReLU, trading saturation for the
risk of killing neurons on negative inputs; Maas et
al.~\cite{maas2013} and Clevert et al.~\cite{clevert2015}
addressed this with LeakyReLU and ELU, and Klambauer et
al.~\cite{klambauer2017} introduced SELU with a self-normalising
property. More recently GELU~\cite{hendrycks2016} became the
transformer default, Swish~\cite{ramachandran2017} was found by
automated search, and Mish~\cite{misra2020} outperformed both on
vision benchmarks; surveys~\cite{nwankpa2018,apicella2021}
catalogue the growing range. All were developed and tested in
feedforward, convolutional or transformer settings, leaving their
behaviour inside gated recurrent dense layers untested.

Within sentiment analysis the IMDb dataset~\cite{maas2011} is the
standard binary benchmark; Tang et al.~\cite{tang2015} validated
GRU for document-level sentiment and Socher et
al.~\cite{socher2013} contributed the SST treebank. Several
studies addressed the activation question partially. Eger et
al.~\cite{eger2018} compared 21 activations across NLP tasks but
used basic architectures and predated Mish. Farzad et
al.~\cite{farzad2019} swept 23 functions at the LSTM gate level on
IMDb with no GRU or bidirectional variants. Essai Ali et
al.~\cite{essaiAli2022} engineered 26 activations for LSTM, but
outside sentiment and without GELU, Swish or Mish. Zhu and
Tan~\cite{zhu2022} tested a custom activation on LSTM and GRU for
IMDb against only ReLU and SELU. Ahmed et al.~\cite{ahmed2025}
tested three classical pairings in a BiLSTM on IMDb, reached
88.40\%, and called for broader architectures and modern
functions.

Four gaps remain open, and the fourth subsumes the others. No
study has swept activation pairings exhaustively rather than
testing a few hand-picked combinations. No study has compared
across LSTM, GRU, and their fully bidirectional variants under
identical conditions. No study has benchmarked GELU, SiLU, or Mish
inside gated recurrent dense layers for sentiment classification.

Most importantly, \emph{none of the works above repeats any
configuration}. Eger et al.~\cite{eger2018}, Farzad et
al.~\cite{farzad2019}, Essai Ali et al.~\cite{essaiAli2022}, Zhu
and Tan~\cite{zhu2022} and Ahmed et al.~\cite{ahmed2025} each
report a single run per setting, and none reports a standard
deviation, a confidence interval, or a seed. Their rankings are
therefore single draws from a stochastic process whose spread is
unknown. Reproducibility studies in other areas of deep learning
have found such rankings to be fragile, but the question has not
been put to activation functions in gated recurrent networks. This paper addresses all four gaps, and treats
the fourth as its primary contribution.
...
We trained 2{,}000 models: all 100 ordered pairs of ten activation
functions, in four gated recurrent architectures, each replicated
under five seeds. The question was not which activation function
wins, but whether that question is answerable as it is usually
posed.

Five findings follow. \emph{First}, the random seed explains
58.7\% of the variance in test accuracy, the architecture 26.7\%,
and the activation pair 3.7\%: seed variation dominates every
controlled factor combined. \emph{Second}, single-seed rankings of
activation pairs do not reproduce. Across four independently swept
architectures the correlation between the single-seed ordering of
100 cells and their replicated means is $-0.174$, $-0.059$,
$-0.041$ and $+0.055$, with average rank movement above 32 places.
The ten best configurations of 400 span 0.12 points against
intervals up to $\pm$0.50, making them statistically
indistinguishable. \emph{Third}, the architectural
conclusion is large and robust: GRU exceeds LSTM by 0.79 points
($p\approx2\times10^{-125}$, $d=1.19$) and supplies all fifty
highest-scoring configurations, while layer-2 bidirectionality
contributes 0.19\% of variance and nothing measurable for GRU.
\emph{Fourth}, GRU is also the more reliable family, with about
40\% lower seed-to-seed standard deviation, and all eight
high-variance cells in the grid are LSTM-based. \emph{Fifth}, one
activation effect survives replication: smooth functions reach
73.70\% validation accuracy after one epoch against 61.05\% for
the ReLU family, 2.9 times the seed standard deviation. The
identity of the best pair, the cross-architecture inversion of
rankings, and the dead-neuron penalty for dense-layer ReLU do not.

For practitioners: choose GRU, prefer a smooth activation if the
budget is measured in epochs, and do not tune the pair further. For
the field: report variance.

Future work should test whether this variance split holds on other
sentiment corpora. SST-2 and Amazon Reviews are the natural next
targets, and would establish whether the architectural advantage
generalises across domains. The same question applies to
attention-based models, where activation choice in feed-forward
blocks follows the same single-run reporting convention.
