...
Every model shares one backbone: an embedding
layer (vocabulary 5,000, dimension 128), a bidirectional
recurrent Layer~1 (64 units per direction), a recurrent
Layer~2 (32 units per direction), Dropout(0.4), Dense Layer~1
(64 units) with Act$_1$, Dense Layer~2 (32 units) with
Act$_2$, and a single output unit trained with
\texttt{BCEWithLogitsLoss}. Two architectural factors are
varied on top of it: the recurrent cell type (LSTM or GRU)
and whether Layer~2 is bidirectional. Crossing them gives the
four backbones of Table~\ref{tab:architectures}, referred to
throughout as BiLSTM+LSTM, BiGRU+GRU, BiLSTM+BiLSTM and
BiGRU+BiGRU; the dense-layer activation pair (Act$_1$,
Act$_2$) is swept within each.

\begin{table}[htbp]
  \caption{The Four Recurrent Backbones}
...
Every setting in Table~\ref{tab:hyperparams} was held fixed
across all 2{,}000 runs, so that within a backbone only the
activation pair and the random seed vary. The architecture
parameters (vocabulary, embedding, layer sizes, dropout)
replicate Ahmed et al.~\cite{ahmed2025} exactly. The training
parameters (optimizer, learning rate, batch size, scheduler,
clipping) are ours, as Ahmed et al. did not report them.
After training, accuracy, precision, recall and
F1-score were measured on the held-out 10,000-review test set.

\subsection{Experimental Design and Replication Protocol}
\label{sec:replication}

The design is a full factorial: the four backbones crossed with
all $10\times10 = 100$ ordered activation pairs give $400$
configurations, and every one of them is retrained under five
seeds, $\{10,20,30,40,50\}$, for $2{,}000$ models in total. The
seed is applied immediately before the model and its data
loaders are constructed, setting \texttt{random}, \texttt{numpy},
\texttt{torch} and \texttt{torch.cuda} with
\texttt{cudnn.deterministic=True}, so a given (backbone, pair,
seed) is exactly reproducible. Runs were executed five at a
time, one process per seed; sampled configurations re-run
serially gave identical metrics.
...
is therefore that of retraining the same model on the same
data --- the quantity a single-run comparison assumes to be
negligible. Split variance would only inflate it, so these
estimates are conservative. We attribute it by partitioning the
total sum of squares of test accuracy over the 2{,}000 runs among
the backbone, Act$_1$, Act$_2$, the Act$_1\times$Act$_2$
interaction, and a residual equal to the pooled within-cell
seed variation.

The sweep records only final accuracy, so convergence speed is
measured in a separate re-run: the ten homogeneous pairs (X+X)
on BiGRU+GRU, logged per epoch under five seeds; the homogeneous
pairing isolates each activation without a partner confound.
...
therefore impossible, and we adopt the fully documented settings
of Table~\ref{tab:hyperparams} instead. Their 88.40\% is a single
run reported without a seed or a standard deviation, so we treat
it as a reference point rather than a target and compare it
against our replicated means in Section~\ref{sec:results}.
%------------------------------------------------------------------------------
