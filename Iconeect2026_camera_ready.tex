...
Training seeds account for 58.7\% of the variation in test
accuracy, compared with 26.7\% for architecture and 3.7\% for
activation pairs. In this experiment, accuracy varies more
between training runs than with either architecture or
activation choice alone.

GRU averages 0.79 percentage points above LSTM, with Cohen's
$d=1.19$. Architecture means change by at most 0.18 points
between the single-seed and five-seed results. Making the
second recurrent layer bidirectional accounts for 0.19\% of
the variance. It adds 0.13 points in LSTM ($p=0.017$), while
the measured change in GRU rounds to 0.00 points ($p=0.858$).
The GRU comparison therefore shows no clear accuracy gain
from a bidirectional second layer.

Within BiGRU+GRU, the five-seed means of the 100 activation
pairs span 0.69 percentage points. The median standard
deviation across seeds is 0.33 points, showing that variation
between runs is substantial compared with the differences
between pairs. The full 0.69-point spread is also smaller
than the 0.79-point gap between GRU and LSTM. These comparisons
show a clearer difference between the two model families
than between individual activation pairs.
